...
class Traces(Resource):
    ...
    def get(self, appName=None):
        try:
            if appName:
                query = f"SELECT * FROM otel_traces WHERE ServiceName = '{appName}' AND StatusCode = 'STATUS_CODE_ERROR' ORDER BY Timestamp DESC"
            else:
                query = "SELECT * FROM otel_traces WHERE StatusCode = 'STATUS_CODE_ERROR' ORDER BY Timestamp DESC"
