...
const TimeFrame = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (timeParams.from && timeParams.to) {
        ...
        const matchedKey = Object.entries(timeRanges).find(
          ([_, value]) => value?.toString() === timeParams.days
        )?.[0];
        if (matchedKey) {
  ...
  const handleTimeframeChange = (range: keyof typeof timeRanges) => {
    try {
      setActiveTab(range);
  ...
  return (
    <div className="flex items-center gap-2 bg-blue-600 p-2 rounded-3xl">
      <div className="flex gap-1 bg-primary-700 rounded-md p-1">
        {(Object.keys(timeRanges) as Array<keyof typeof timeRanges>).map(
          (range) => (
            <button
              key={range}
